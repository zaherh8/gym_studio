defmodule GymStudioWeb.Client.GoalsLive do
  @moduledoc """
  Client fitness goals page — set goals, track progress, and celebrate achievements.
  """
  ...
  def mount(_params, _session, socket) do
    ...
    socket =
      socket
      |> assign(page_title: "Fitness Goals")
      |> assign(status_filter: "all")
      |> assign(editing_progress: nil)
      |> assign_goals(user.id)
      |> assign_new_form()
  ...
  def handle_event("achieve", %{"id" => id}, socket) do
    ...
    if goal.client_id != user.id do
      ...
      {:noreply,
       socket
       |> assign_goals(user.id, socket.assigns.status_filter)
  ...
  defp assign_goals(socket, client_id, status_filter \\ "all") do
    opts = if status_filter in [nil, "", "all"], do: [], else: [status: status_filter]
  ...
  def render(assigns) do
    ...
    <div class="min-h-screen bg-gray-100 py-8">
      <div class="container mx-auto px-4">
        <div class="mb-6">
          <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Fitness Goals</h1>
          <p class="text-gray-600 mt-1">Set goals and track your progress</p>
          <.link navigate={~p"/client/progress"} class="btn btn-ghost btn-sm mt-2">
            ← Back to Progress
          </.link>
        </div>
        ...
        <%= if @goals == [] do %>
          <div class="bg-white rounded-2xl shadow-lg p-8 text-center">
            ...
            <p class="text-gray-400 mt-2">Create your first goal above to start tracking!</p>
          </div>
        <% else %>
