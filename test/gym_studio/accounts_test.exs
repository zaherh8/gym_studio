defmodule GymStudio.AccountsTest do
  use GymStudio.DataCase

  alias GymStudio.Accounts

  import GymStudio.AccountsFixtures
  alias GymStudio.Accounts.{User, UserToken}
  ...
  describe "register_user/1" do
    ...
    test "registers users with phone_number and password" do
      phone_number = unique_phone_number()
      {:ok, user} = Accounts.register_user(valid_user_attributes(phone_number: phone_number))
  ...
  describe "login_user_by_magic_link/1" do
    # Note: With phone-based auth, users have passwords by default,
    # so magic link login raises for unconfirmed users with password.
    # We keep these tests for confirmed users who can still use magic links.
    ...
    test "raises when unconfirmed user has password set" do
      user = unconfirmed_user_fixture()
      # User now has password by default from registration
      {encoded_token, _hashed_token} = generate_user_magic_link_token(user)
